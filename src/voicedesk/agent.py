...
def build_system_prompt(today: date, lang: str = DEFAULT_LANG) -> str:
    ...
    prompt = (
        ...
        "Before you cancel or reschedule an appointment, read the appointment you "
        "found back to the caller (the date, time and reason) and get an explicit "
        "yes. Cancelling is destructive — never do it without confirmation. Once the "
        "caller confirms, call the tool immediately; do not ask twice. "
